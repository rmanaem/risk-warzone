...
GameStarter::GameStarter(const GameStarter &original)
{ //copy constructor
...
int GameStarter::selectMap()
{
    ...
    while (!isInputCorrect)
    {
        ...
        { //handles invalid inputs (i.e. 0, x>listOfMaps.size() and non-int input)
            if (cin.fail())
            {
                cin.clear();
...
void GameStarter::selectNumOfPlayers()
{
    ...
    while (!isInputCorrect)
    {
        ...
        { //hanldes invalid inputs (i.e. 0, tempNumOfPlayers<2 or tempNumOfPlayers>5 or non-int input)
            if (cin.fail())
            {
                cin.clear();
                cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
