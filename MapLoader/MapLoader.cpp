...
Map MapLoader::parseMap(std::string map)
{
    ...
    while (getline(MyReadFile, line))
    {
        ...
        if (line.find("[countries]") != std::string::npos)
        {
            ...
            while (getline(MyReadFile, countries))
            {
                ...
                if (!(countries.find("[borders]") == -1 && countries[0] != '\r'))
                {
                    line = countries;
                    break;
                }
    ...
    {
        cout << territory->getData().getTerritoryName() + " belongs to " + territory->getData().getContinent()->getContinentName() + " has the following edges:" << endl;
...
ConquestFileReaderAdapter::ConquestFileReaderAdapter(const ConquestFileReaderAdapter &original)
{ //copy constructor
